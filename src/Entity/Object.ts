...
export default class ObjectEntity extends Entity {
    ...
    public applyPhysics() {
        /* need to be able to garantee both players are shot out
        if (!this.isViewed) {
        ...
        */
        ...
        if (!(this.physicsData.values.flags & PhysicsFlags.canEscapeArena)) {
            ...
            yPos: {
                ...
                else if (this.positionData.values.y > arena.arenaData.values.bottomY - adjust + arena.ARENA_PADDING) this.positionData.y = arena.arenaData.values.bottomY - adjust + arena.ARENA_PADDING;
                else break yPos;
