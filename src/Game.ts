...
import CompLobbyArena from "./Gamemodes/CompLobby";
...
type DiepGamemodeID = "ffa" | "sandbox" | "teams" | "4teams" | "mot" | "dom" | "maze" | "tag" | "survival" | "testing" | "spike" | "domtest" | "jungle" | "factest" | "ball" | "comp";
...
export default class GameServer {
    ...
    public constructor(gamemode: DiepGamemodeID, name: string | "*", endpoint: string) {
    ...
    //Returns verified players in lobby
    public verifiedPlayerCount(){
        let count = 0;
        for(const client of this.clients){
            if(client.verifiedUser == true && Entity.exists(client.camera?.cameraData.values.player)) count++;
        }
        return count;
    }

    //Returns Average elo of the lobby
    public getAverageElo(){
        let sum = 0.0;
        let count = 0;
        for(const client of this.clients){
            if(client.elo >= 0 && client.verifiedUser ==true){
                sum += client.elo;
                count++;
            }
        }
    ...
    private tickLoop() {
        ...
        this.lobby.tick(this.tick); //ticking lobby
