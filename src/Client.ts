...
//importing whitelist
import * as whitelist from './comp_addons/whitelist';
//importing GameManager
import{ getAllGames } from "./comp_addons/GameManager";
//importing lobby
import { Lobby } from './comp_addons/Lobby';
...
export default class Client {
    ...
    public onClose(code: number, message: ArrayBuffer) {
        //removing from lobby DEAD TANK
        console.log("disconnected: " + this.clientUsername)
        this.game.lobby.removeClient(this);
    ...
    private handleIncoming(header: number, data: Uint8Array): void {
        ...
        switch (header) {
            case ServerBound.Input: {
                ...
                let flags = this.inputs.flags = r.vu() | this.inputs.cachedFlags;

                //Stuns player
                if(this.stun === true){
                    //disables player from moving
                    flags &= ~InputFlags.left;
                    flags &= ~InputFlags.right;
                    flags &= ~InputFlags.down;
                    flags &= ~InputFlags.up;
                    this.inputs.flags = flags;
                }
            ...
            case ServerBound.Spawn: {
                ...
                // Read the combined "username|password" string from the packet
                const rawString = r.stringNT().slice(0, 16);
                // Split into username and password
                let [username, password] = rawString.split('|');
                let name = "";

                //R E M O V E
                console.log(username + " wants to join");

                // Authenticate using the whitelist
                let denied = false;
                if (!whitelist.authenticate(username, password)) {

                    if(!whitelist.findPlayer(username)){
                        name = this.kickWithMessage("ACOUNT DOES NOT EXIST");
                    }else{
                        name = this.kickWithMessage("INCORRECT PASSWORD");
                    }
                    denied = true;

                }else{ //authentication true, finding lobby

                    //getting all games
                    const games = getAllGames();
                    let lowest = 100000;
                    let lowestGame = games[0];
                    for(const game of games){
                        console.log(game.name + ": " + game.verifiedPlayerCount()); //CONSOLE LOG
                        if(game.verifiedPlayerCount() <= 10){ //if lobby isnt full
                            const average = game.getAverageElo(); //finding lowest average
                            if (average >= 0 && average < lowest){
                                lowest = average;
                                lowestGame = game;
                            }
                        }else{
                            games.splice(games.indexOf(game), 1); //removing full lobbies from list
                        }
   
                    }
                    if(games.length == 0){ //if all lobbies are full
                        username = this.kickWithMessage("All Lobbies Full :(");
                    }else if(lowest == 100000){ //suggesting next empty lobby
                        if(this.isSameGame(games[0])){
                            //client too be verified
                        }else{
                            name = this.kickWithMessage("Join Lobby " + games[0].name);
                            denied = true;
                        }
                    }else{ //suggest lobby matching elo
                        if(this.isSameGame(lowestGame)){
                            //client too be verified
                        }else{
                            name = this.kickWithMessage("Join Lobby " + lowestGame.name);
                            denied = true;
                        }
                    }
                }

                if(denied === false && !this.game.lobby.isUniquePlayerAccount(username)){ //if we passed, see if its a repeat account
                    name = this.kickWithMessage("ONLY 1 TAB ALLOWED");
                    denied = true;
                }else if(denied === false){ //verifying client after final check
                    name = this.verifyClient(username);
                }
                ...
                if(denied) tank.setTank(DevTank.Empty); //set to empty tank if not valid user
    ...
    public setSpectate(tf: boolean, fieldFactor?: number){
        if(tf){
            if(this.camera){
                this.camera.cameraData.values.flags |= CameraFlags.usesCameraCoords;
                this.camera.cameraData.cameraX = 0;
                this.camera.cameraData.cameraY = 200;
                if(fieldFactor)
                    this.camera.setFieldFactor(fieldFactor);
            }
        }else{
            if(this.camera){
                this.camera.cameraData.values.flags ^= CameraFlags.usesCameraCoords;
                const player = this.camera.cameraData.player;
                if(player instanceof TankBody)
                    this.camera.setFieldFactor(player.definition.fieldFactor);
            }
        }
    }

    public verifyClient(username: string){
        this.verifiedUser = true;
        this.elo = whitelist.findPlayer(username)?.elo ?? -1.0;
        this.clientUsername = username;
        return username;
    }

    //checks wether that game is this game (lmfao)
    public isSameGame(inputGame: GameServer): boolean{
        if(inputGame.name == this.game.name)
            return true;
        return false;
    }

    //notifys player of message and returns that message as a string
    public kickWithMessage(message: string): string{
        this.notify(message, 0x000000, 5000);
    ...
    public possess(ai: AI) {
        ...
        if (this.camera?.cameraData.values.player instanceof ObjectEntity) {
            ...
            this.camera.cameraData.values.player.styleData.values.color = 0; // changed this cause of error -1
        ...
        this.camera.cameraData.tank = 54; //changed this cause of error 53
