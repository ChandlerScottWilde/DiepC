...
const whitelist = __importStar(require("./comp_addons/whitelist"));
...
class Client {
            ...
            case 1: {
                ...
                if ((flags & 32)) {
                    if (this.accessLevel >= 2) {
                        ...
                        player.setTank(player.currentTank < 0 ? Enums_1.Tank.Basic : -1);
                    }
            ...
            case 2: {
                ...
                if (!whitelist.authenticate(username, password)) {
                    ...
                    for (const game of games) {
                        console.log(game.name + ": " + game.verifiedPlayerCount());
                        if (game.verifiedPlayerCount() <= 10) {
                            const average = game.getAverageElo();
                ...
                if (denied === false && !this.game.lobby.isUniquePlayerAccount(username)) {
    ...
    setSpectate(tf, fieldFactor) {
        if (tf) {
            if (this.camera) {
                this.camera.cameraData.values.flags |= 1;
    ...
    possess(ai) {
        ...
        if (this.camera?.cameraData.values.player instanceof Object_1.default) {
            ...
            this.camera.cameraData.values.player.styleData.values.color = 0;
        ...
        this.camera.cameraData.tank = 54;
        for (let i = 0; i < Enums_1.StatCount; ++i)
